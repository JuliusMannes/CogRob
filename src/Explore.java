...
public Explore(float a_fFearDistance, float a_fClearDistance, UltraSonicSensor a_pLeftSensor, UltraSonicSensor a_pRightSensor, float a_fSpeedModifier) { 
	//Set variables
...
public boolean takeControl() {
	...
	if (!m_bInControl && (m_pLeftSensor.DetectSample() < m_fFearDistance || m_pRightSensor.DetectSample() < m_fFearDistance)) {
		//Log
		System.out.println("Object detected, initializing Exploration!");
		return true;
	} 
	//Otherwise
	return false;
...
public void action() {
	...
	while( !m_bSuppressed && (DriveLeft || DriveRight || FirstGo)) {
		...
		if(!DriveLeft && Left < m_fFearDistance) {
			Motor.D.setSpeed(m_fSpeedModifier * Left);
			Motor.D.backward();
			DriveLeft = true;
			...
			Motor.D.setSpeed((int) (m_fSpeedModifier * (Left-0.01f)));
		}
		
		//Check right side
		if(!DriveRight && Right < m_fFearDistance) {
			Motor.A.setSpeed(m_fSpeedModifier * Right);
			Motor.A.backward();
			DriveRight = true;
			...
			Motor.A.setSpeed((int) (m_fSpeedModifier * (Right-0.01f)));
		}
	 }
